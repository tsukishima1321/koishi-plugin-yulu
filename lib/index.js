    ...
    ctx.command('yulu/yulu_select [...rest]').alias("语录")
        ...
        if (options.id) {
            ...
            if (options.list) {
                ...
                if (!await checkFile((0, path_1.join)(cfg.dataDir, String(options.id)))) { //语录文件下载失败
            ...
            if (!options.list) {
                ...
                if (!await checkFile((0, path_1.join)(cfg.dataDir, String(find.id)))) { //语录文件下载失败
            ...
            for (var i = 0; i < listeningQueue.length; i++) {
                if ((!session.guildId || session.guildId == listeningQueue[i].group) && session.event.user.id == listeningQueue[i].user) {
                    if (session.event.message.elements[0].type == "img") {
                        ...
                        if (!await checkFile((0, path_1.join)(cfg.dataDir, String(listeningQueue[i].id)))) { //语录文件下载失败
                            console.warn(`语录${src}下载失败`);
                            async function retry(ms) {
                                return new Promise((resolve, reject) => {
                                    setTimeout(async () => {
                                        await getfileByUrl(src, String(listeningQueue[i].id), cfg.dataDir);
                                        resolve(await checkFile((0, path_1.join)(cfg.dataDir, String(listeningQueue[i].id))));
                                    }, ms);
                                });
                            }
                            var flag = false;
